...
if __name__ == "__main__":
    ...
    prior = pyabc.Distribution(kappa=pyabc.RV("uniform", 0, 10),
                               ...
                               damp_time=pyabc.RV("rv_discrete", values=(discrete_domain_1, [1 / 49] * 49)),
    ...
    def model(pars):
        ...
        try:
            df = pd.read_csv(f"output/{curr_seed}_comps.csv", delimiter=" ", index_col=False)
    ...
    abc = pyabc.ABCSMC(model, prior, distance, transitions=transition, population_size=300, sampler=sampler)

    db_path = os.path.join('output', "abm.db")
    observed_prev = [329, 2350, 6136, 6279, 4679, 3357, 2421, 1757]
